...
if RUN_SETUP == 'mg_studies':
    ...
    out_tag = "lobster_{tstamp}".format(timestamp_tag)
    output_path  = "/store/user/$USER/CentralProduction/tests/{tag}/{ver}".format(tag=out_tag,ver=version)
    workdir_path = "/tmpscratch/users/$USER/CentralProduction/tests/{tag}/{ver}".format(tag=out_tag,ver=version)
    plotdir_path = "~/www/lobster/CentralProduction/tests/{tag}/{ver}".format(tag=out_tag,ver=version)
else:
...
tzq_info = {
    ...
    'gridpack_location': os.path.join(input_path_full,"awightma/gridpack_scans/central_gridpacks"),
    'gridpack_name': 'tzq_ll_4f_ckm_NLO_slc6_amd64_gcc481_CMSSW_7_1_30_tarball.tar.xz',
...
for idx,gp_info in enumerate(gridpacks):
    ...
    lhe = Workflow(
        label='lhe_step_{tag}'.format(tag=label_tag),
        command=' '.join(cmd),
